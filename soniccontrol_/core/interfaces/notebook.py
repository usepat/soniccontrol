...
from soniccontrol.core.interfaces.rootchild import RootChild
...
class RootNotebook(ttk.Notebook, Resizable):
    ...
    def forget_tabs(self, frames: Optional[RootChild] = None) -> None:
        ...
        frames = self.currently_managed_tabs.copy() if frames is None else frames
        logger.debug(f"Last focused tab: {self._last_focused_tab}")
        logger.debug(f"Tabs: {self.tabs()}")
        logger.debug(f"Frames: {frames}")
        for frame in frames:
            logger.debug(f"forgetting {frame}")
    ...
    def forget_and_add_tabs(self, tabs: Iterable[RootChild]) -> None:
        ...
        self.add_tabs(tabs)
        try:
            logger.debug(f"selecting {self._last_focused_tab}")
            self.select(self._last_focused_tab)
